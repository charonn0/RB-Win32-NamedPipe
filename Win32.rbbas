...
Protected Module Win32
	#tag ExternalMethod, Flags = &h21
		Private Declare Function ConnectNamedPipe Lib "Kernel32" (hPipe As Integer, Overlapped As Ptr) As Boolean
	...
	#tag Method, Flags = &h1
		Protected Function CreateNamedPipe(PipeName As String, OpenMode As Integer, MaxInstances As Integer = - 1, OutBufferSize As Integer = 512, InBufferSize As Integer = 512, DefaultTimeout As Integer = - 1, PipeMode As Integer = 0) As BinaryStream
		  ...
		  Dim hFile As Integer = Win32.CreateNamedPipe("\\.\pipe\" + PipeName, OpenMode, PipeMode, MaxInstances, OutBufferSize, InBufferSize, DefaultTimeout, Nil)
		  err = Win32.GetLastError()
		  Dim stream As New BinaryStream(hFile, BinaryStream.HandleTypeWin32Handle)
		  
		  If err = 0 Then
		    If Not Win32.ConnectNamedPipe(stream.Handle(BinaryStream.HandleTypeWin32Handle), Nil) Then 'ConnectNamedPipe blocks until a client connects (TODO: async mode)
		      stream = Nil
		    End If
		  Else
		    stream = Nil
